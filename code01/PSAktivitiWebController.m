...
@interface PSAktivitiWebController ()
...
@implementation PSAktivitiWebController
...
- (void)viewDidLoad
{
    ...
    [button addTarget:self action:@selector(goToAktivitiController) forControlEvents:UIControlEventTouchUpInside];
    ...
    NSLog(@"url: %@", self.urlNews);
    //http://stackoverflow.com/questions/2028379/uiwebkit-error-101
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        NSString *encodedString = [self.urlNews stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error
{
    ...
    if ([error code] != NSURLErrorCancelled) {
        ...
        [[self navigationItem] setTitle:@"Aktiviti"];
